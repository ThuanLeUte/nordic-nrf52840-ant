...
uint8_t ant_msg_builder_calculate_crc(uint8_t *p_data, uint16_t len);
...
void ant_msg_builder(ant_msg_transfer_t *p_transfer)
{
  ...
  ant_msg_buffer[0] = (uint8_t)p_transfer->msg_id;
  ant_msg_buffer[1] = p_transfer->in_len;
  ant_msg_buffer[2] = p_transfer->msg_id;
  
  // Message content
  memcpy(&ant_msg_buffer[3], p_transfer->in_data, p_transfer->in_len);

  // Check sum
  crc = ant_msg_builder_calculate_crc(ant_msg_buffer, 3 + p_transfer->in_data);
  ant_msg_buffer[2 + p_transfer->in_data] = crc;
  // ===================================== }

  // Get output data for sending
  p_transfer->out_len = p_transfer->in_data + 4;
